...
class EnhancedEnricherIntegration:
    ...
    def enrich_lead_comprehensive(self, lead_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Step 2.5: Fill missing contact fields
            contact_data = self._enrich_contact_fields(lead, content_result)
            
            # Step 3: Combine results and update database
            enrichment_data = self._combine_enrichment_results(content_result, traits_result, contact_data)
    ...
    def _enrich_contact_fields(self, lead: Lead, content: Dict[str, Any]) -> Dict[str, Any]:
        """Enrich missing contact fields for the lead."""
        logger.info(f"📞 Enriching contact fields for {lead.name}")
        
        contact_data = {}
        
        # 1. Fill missing LinkedIn URL
        if not hasattr(lead, 'linkedin_url') or not lead.linkedin_url or lead.linkedin_url.strip() == "":
        ...
        # 2. Extract phone number from website content
        if not hasattr(lead, 'phone') or not lead.phone:
            phone = self._extract_phone_from_content(content)
            if phone:
                contact_data['phone'] = phone
                contact_data['phone_source'] = 'website_extraction'
                logger.info(f"   ✅ Found phone number: {phone}")
        
        # 3. Ensure company website is set
        if not hasattr(lead, 'company_website') or not lead.company_website:
            if hasattr(lead, 'website') and lead.website:
                contact_data['company_website'] = lead.website
            else:
                website = self._generate_company_website(lead)
                if website:
                    contact_data['website'] = website
                    contact_data['company_website'] = website
                    logger.info(f"   ✅ Generated website: {website}")
        
        # 4. Add missing location data
        if not hasattr(lead, 'location') or not lead.location:
            location = self._extract_location_from_content(content)
            if not location:
                location = 'North America'  # Default
            contact_data['location'] = location
            logger.info(f"   ✅ Set location: {location}")
        
        # 5. Add missing industry data
        if not hasattr(lead, 'industry') or not lead.industry:
            industry = self._infer_industry_from_content(content)
            if industry:
                contact_data['industry'] = industry
                logger.info(f"   ✅ Inferred industry: {industry}")
        
        # 6. Add missing company size
        if not hasattr(lead, 'company_size') or not lead.company_size:
            company_size = self._infer_company_size_from_content(content)
            if company_size:
                contact_data['company_size'] = company_size
                logger.info(f"   ✅ Inferred company size: {company_size}")
        
        # 7. Add engagement readiness flags
        contact_data.update({
            'ready_for_engagement': True,
            'needs_enrichment': False,
            'enriched': True,
            'enrichment_method': 'enhanced_integration',
            'contact_enriched_at': datetime.now().isoformat()
        })
        
        return contact_data
    
    def _generate_linkedin_url(self, lead: Lead) -> Optional[str]:
        """Generate LinkedIn URL based on lead name."""
        name = getattr(lead, 'name', '') or getattr(lead, 'full_name', '')
        if not name:
            return None
    ...
    def _extract_phone_from_content(self, content: Dict[str, Any]) -> Optional[str]:
        ...
        phone_patterns = [
            ...
            r'\b\(\d{3}\)\s?\d{3}[-.\s]?\d{4}\b',   # (123) 123-1234
            r'\b\+1[-.\s]?\d{3}[-.\s]?\d{3}[-.\s]?\d{4}\b',  # +1 format
            r'\b\d{3}\.\d{3}\.\d{4}\b'  # 123.123.1234
        ]
    ...
    def _generate_company_website(self, lead: Lead) -> Optional[str]:
        """Generate company website based on company name."""
    ...
    def _extract_location_from_content(self, content: Dict[str, Any]) -> Optional[str]:
    ...
    def _infer_industry_from_content(self, content: Dict[str, Any]) -> Optional[str]:
        """Infer industry from website content."""
        text = content.get('text', '').lower()
        title = content.get('page_title', '').lower()
        
        all_content = f"{text} {title}"
        
        industry_keywords = {
            'Technology': ['software', 'tech', 'saas', 'platform', 'digital', 'app'],
            'Healthcare': ['health', 'medical', 'healthcare', 'clinic', 'hospital'],
            'Financial Services': ['finance', 'financial', 'bank', 'investment', 'accounting'],
    ...
    def _infer_company_size_from_content(self, content: Dict[str, Any]) -> Optional[str]:
        """Infer company size from website content."""
        text = content.get('text', '').lower()
        
        # Look for size indicators
        if any(term in text for term in ['enterprise', 'large', 'corporation', 'global']):
            return '1001-5000'
        elif any(term in text for term in ['team of', 'staff of', 'employees']):
            return '51-200'
        elif any(term in text for term in ['startup', 'founded', 'small']):
            return '11-50'
        
        return '51-200'  # Default medium size
    
    def _combine_enrichment_results(self, content: Dict[str, Any], traits: Dict[str, Any], contact_data: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        if contact_data:
            enrichment_data.update(contact_data)
