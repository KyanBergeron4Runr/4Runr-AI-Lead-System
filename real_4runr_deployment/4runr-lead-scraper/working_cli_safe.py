#!/usr/bin/env python3
"""
Working CLI for 4runr-lead-scraper (Safe Version)

A fully functional command-line interface without Unicode characters.
"""
import sqlite3
import sys
import os
...
class WorkingCLI:
    ...
    def list_leads(self, limit: int = 10, status: Optional[str] = None):
        ...
        try:
            ...
            for name, email, company, lead_status, enriched, linkedin in leads:
                enriched_status = "[E]" if enriched else "[ ]"
                print(f"{enriched_status} {name}")
                print(f"   Email: {email or 'No email'}")
                print(f"   Company: {company or 'No company'}")
    ...
    def test_config(self):
        ...
        if self.db_path.exists():
            print(f"[OK] Database: Found at {self.db_path}")
        else:
            print(f"[MISSING] Database: Not found at {self.db_path}")
    ...
    def help(self):
        ...
        print()
        print("Examples:")
        print("  python run_cli.py stats")
        print("  python run_cli.py list 20")
        print("  python run_cli.py list 10 --status='Ready for Outreach'")
        print("  python run_cli.py search 'TechCorp'")
        print("  python run_cli.py backup")
