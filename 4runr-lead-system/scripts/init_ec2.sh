#!/bin/bash
# Script to initialize an EC2 instance for the 4Runr Lead System
...
PROJECT_DIR="/home/ubuntu/4runr-lead-system"
mkdir -p $PROJECT_DIR
...
cat > /etc/logrotate.d/4runr-lead-system << EOF
...
# Set up cron job for the scraper
print_header "Setting up cron job"
CRON_JOB="0 */6 * * * cd $PROJECT_DIR && docker-compose run --rm backend npm run scrape >> $PROJECT_DIR/logs/scraper.log 2>&1"
(crontab -u ubuntu -l 2>/dev/null || echo "") | grep -v "docker-compose run --rm backend" | { cat; echo "$CRON_JOB"; } | crontab -u ubuntu -
...
echo "The 4Runr Lead System has been set up on this EC2 instance."
echo ""
echo "Next steps:"
...
echo ""
echo "The scraper will run automatically every 6 hours via cron."
echo "You can also run it manually: cd $PROJECT_DIR && docker-compose run --rm backend npm run scrape"
echo ""
